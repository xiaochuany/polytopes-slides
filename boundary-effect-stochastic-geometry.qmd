...
- $L_n = \max_{x} dist(x, \mathcal X\setminus \{x\})$
...
- at each $\varepsilon_1 r_n$ lattice point, EITHER non-activated OR at least two points within $(1+2\varepsilon_1)r_n$
$$\mathbb P[ . \cup ..]\le  1 - \eta \mathbb P[\{..\}^c]$$
...
$$ \mathbb (1- (\eta/k!) (n ar^d)^k e^{- n ar^d} )^{c_1 r_n^{-b}} = O(\exp(-   n^{b/d - ac}))\to 0$$
provided $c< (b/d)/a$
